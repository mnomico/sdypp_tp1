"""Hit #5 — Nodo C con mensajes en formato JSON.

Mismo nodo bidireccional del Hit #4, pero los saludos y las respuestas se
serializan y deserializan como JSON (ver `comun.mensajes`) en lugar de viajar
como texto plano. Cada mensaje ocupa una línea (JSON Lines), de modo que el
delimitador del Hit #1 sigue sirviendo para separarlos.

  C1 --{"tipo":"saludo",...}--> C2      (C1 cliente, C2 servidor)
  C1 <--{"tipo":"saludo",...}-- C2      (C2 cliente, C1 servidor)
"""
...
PUERTO_POR_DEFECTO = config.entero("TP1_PUERTO_HIT5", 9005)
...
class NodoC:
    ...
    def construir_respuesta(self, saludo):
        """Responde a un saludo ya deserializado.

        Con JSON el remitente llega en el campo `origen`, así que la respuesta se
        arma con datos estructurados en vez de recortar una cadena de texto.
        """
            ...
            with conexion:
                ...
                while not self._detenido.is_set():
                    ...
                    try:
                        ...
                        # Un mensaje mal formado no debe cortar el canal ni el nodo.
                        with self._lock:
                            self._mensajes_invalidos += 1
                        self._logger.warning(
                    ...
                    self._logger.info(
                        "[entrante] %s de %s: %s",
                        saludo["tipo"], saludo["origen"], saludo["contenido"],
                    )
        ...
        while not self._detenido.is_set():
            try:
                with socket.create_connection((host, puerto), timeout=timeout) as sock:
                    ...
                    self._logger.info(
                        "[saliente] saludo enviado (id=%s, %s bytes): %s",
                        saludo["id"], mensajes.tamano_en_bytes(saludo), saludo["contenido"],
                    )
                    ...
                    self._logger.info(
                        "[saliente] respuesta de %s: %s",
                        respuesta["origen"], respuesta["contenido"],
                    )
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Hit #5 - Nodo C con mensajes JSON"
    )
    ...
    logger, _ = configurar("hit5.nodo_c")
